...
class WCNFFormula(object):
    ...
    #python3 wcnf.py ex2.wcnf file
    def to_13wpm(self):
        formula = WCNFFormula()
        formula.extend_vars(self.num_vars)
        
        for clause in self.soft:
            if len(clause[1]) == 1:  # Afegim les clausules que ja tenen llargada 1
                formula.add_clause(clause[1], clause[0])
                continue
            reif_var = formula.new_var()
            formula.add_clause([-reif_var], clause[0])
            formula.add_clause(clause[1] + [reif_var], 0)
        formula.add_clauses(self.hard)
        Newformula13 = WCNFFormula()
        Newformula13.extend_vars(formula.num_vars)
        for clause in formula.soft:
            Newformula13.add_clause(clause[1], clause[0])
        for clause in formula.hard:
            Newformula13.to_len_3(clause)

        return Newformula13

    def to_len_3(self, clause):
        if len(clause) <= 3:
            increments = [clause[0]] * (3 - len(clause))
            self.add_clause(clause + increments, 0)
        else:
            new_var = self.new_var()#fem la particio de les clausules amb llargada superior 3 de forma recursiva
            self.add_clause(clause[:2] + [new_var], 0)
            self.to_len_3([-new_var] + clause[2:])
