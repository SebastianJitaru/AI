...
def kcluster(rows,distance=pearson,k=4):
    ...
    return (clusters, sum_distances)

#improved kcluster
def improved_kcluster(rows,distance=pearson,k=4, num_executions=10):
    best_config = None
    best_sum_distances = float('inf')
    ...
    return best_config
def main():
    _,_,rows = readfile("blogdata_full.txt")
    total_distances = {}
    for k in range(2, 11):
        clusters, sum_distances = kcluster(rows, k=k)
        total_distances[k] = sum_distances

    for k, distance in total_distances.items():
        print(f"k: {k}, Total Distance: {distance}")
